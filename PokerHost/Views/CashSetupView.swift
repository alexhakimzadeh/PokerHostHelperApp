...
struct CashSetupView: View {
    ...
    @State private var expectedRebuys: Int = 0
    ...
    @State private var loadingLogLines: [String] = ["Shuffling chips and counting rebuys..."]
    ...
    private let loadingMessages: [String] = [
        ...
        "Sweating the river...",
        "Coloring up the short stacks...",
        "Making sure the white chips do not take over...",
        "Counting out blind posts...",
        "Asking the dealer for one more second...",
        "Double-checking the chip race math...",
        "Looking for a stack that does not feel cursed...",
        "Balancing splashy chips with practical chips...",
        "Making change before the next hand starts...",
        "Trying not to hand everyone 50 tiny chips...",
        "Searching for a stack worthy of the main game...",
        "Keeping a few chips behind for the re-entry crowd...",
        "Running the numbers like a very serious pit boss...",
        "Tank-calling a better allocation..."
    ]
    ...
    private var parsedBuyInCents: Int? {
        Money.cents(from: buyInText)
    }

    private var plannedRebuyValueCents: Int {
        guard let buyInCents = parsedBuyInCents else { return 0 }
        return expectedRebuys * buyInCents
    }

    private var suggestedReservePercent: Double? {
        guard totalChipBankValueCents > 0, plannedRebuyValueCents > 0 else { return nil }
        let raw = Double(plannedRebuyValueCents) / Double(totalChipBankValueCents)
        return min(max(raw, 0), 0.70)
    }
    ...
    var body: some View {
        ScrollViewReader { proxy in
            ZStack {
                ...
                if isCalculating {
                    ...
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        Text("Still working. Bigger chip sets can take a moment while we search for the best stack.")
                            .font(.footnote)
                ...
                if let result {
                    ResultsSheetView(
                        recommendation: AllocationRecommendation(
                            label: "Best",
                            summary: result.feasible ? "Best available stack recommendation." : "No exact playable stack found with the current setup.",
                            highlights: [],
                            chips: chips,
                            allocation: result
                        ),
            ...
            .onChange(of: expectedRebuys) { saveCurrentSetup() }
    ...
    private var gameSection: some View {
        CardView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Stepper("Expected Rebuys: \(expectedRebuys)", value: $expectedRebuys, in: 0...20)
                    .foregroundStyle(AppColors.textPrimary)
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    if expectedRebuys > 0 {
                        HStack {
                            Text("Planned rebuy value")
                            Spacer()
                            Text(Money.format(cents: plannedRebuyValueCents))
                                .foregroundStyle(AppColors.textSecondary)
                        }
                        .foregroundStyle(AppColors.textPrimary)

                        if let suggestedReservePercent {
                            HStack {
                                Text("Suggested reserve")
                                Spacer()
                                Text("\(Int((suggestedReservePercent * 100).rounded()))%")
                                    .foregroundStyle(AppColors.textSecondary)
                            }
                            .foregroundStyle(AppColors.textPrimary)

                            Button {
                                reservePercent = suggestedReservePercent
                            } label: {
                                Text("Use Suggested Reserve")
                                    .fontWeight(.semibold)
                                    .frame(maxWidth: .infinity)
                                    .padding(.vertical, 10)
                                    .background(AppColors.card)
    ...
    private func calculate() async {
        ...
        let calculationResult: (updatedChips: [ChipType]?, allocation: AllocationResult) = await Task.detached(priority: .userInitiated) { @Sendable in
            if currentMode == .auto {
                let optimized = ChipAllocator.rankedAuto(
                    chips: currentChips,
                ...
                return (
                    updatedChips: optimized.primaryChips,
                    allocation: optimized.primaryAllocation
                )
            } else {
                let ranked = ChipAllocator.rankedManual(
                    chips: currentChips,
                    players: currentPlayers,
                    buyInCents: buyInCents,
                    reservePercent: currentReservePercent,
    ...
    private func startLoadingMessages() {
        loadingTask?.cancel()
        loadingLogLines = ["Shuffling chips and counting rebuys..."]

        loadingTask = Task {
            var cycleMessages = shuffledLoadingMessages()
            var index = 0

            while !Task.isCancelled {
                if index >= cycleMessages.count {
                    let previous = cycleMessages.last
                    cycleMessages = shuffledLoadingMessages(avoiding: previous)
                    index = 0
                }

                let nextLine = cycleMessages[index]
    ...
    private func shuffledLoadingMessages(avoiding previous: String? = nil) -> [String] {
        guard !loadingMessages.isEmpty else { return [] }

        var shuffled = loadingMessages.shuffled()

        if let previous,
           shuffled.count > 1,
           shuffled.first == previous,
           let nextDistinctIndex = shuffled.firstIndex(where: { $0 != previous }) {
            shuffled.swapAt(0, nextDistinctIndex)
        }
    ...
    private func loadSavedSetup() {
        ...
        expectedRebuys = saved.expectedRebuys
